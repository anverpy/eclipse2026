...
### Added
- Sub-minute cadence during totality (20:15–20:45) for the two metrics people actually watch live — camera brightness and the public dashboard snapshot — without adding Step Functions or any new AWS service: `dgt_camera` and `aggregator`'s handlers now tick internally 3x (~20s apart) per invocation when EventBridge passes `tier: "tight"`, instead of once. Effective cadence during totality goes from 1min/2min to ~20s. `tier` is threaded from `terraform/scheduler.tf`'s schedule locals into each schedule's `input`; a new `aggregator-*-tight` schedule (`rate(1 minute)`, 20:15–20:45) was added so the aggregator is even invoked during that window instead of sitting on its old flat 2min. `ree`/`dgt_traffic`/`aemet`/`trends` are untouched — their source data doesn't resolve any finer than what's already scheduled, so ticking them faster would just be redundant requests for identical values (a real concern for REE's usage terms specifically, see README source #1). Lambda timeouts for `dgt_camera` (60s→90s) and `aggregator` (30s→90s) bumped to give the tick loop room; both still fall back to a single tick with no `tier` (normal-tier invocations, and local dev's `DEFAULT_EVENT`, are unaffected).
- Dashboard frontend pass: removed the "Fuente" filter chip row — it mapped 1:1 onto individual map category tabs (e.g. the AEMET chip only ever did anything in the Clima view), which read as broken paired with the 3-way category switch; every source now always renders in whichever category uses it. "Ciudad" switched from a single-select `<select>` to multi-select chips (same visual style as "Ventana temporal", but toggles independently) — clicking a map marker, city-table row, or chip all toggle that city in/out of the selection instead of replacing it. Added a coverage note under the map that flags when a selected city has no marker in the current category (e.g. Palma has no DGT camera — only 9 mainland provinces have one — so "Efecto eclipse" never places a dot there), so the gap reads as a known data-coverage limit, not a broken page. Fixed a real bug where selecting any city force-scrolled the whole page down to the "Todas las fuentes en el tiempo" section, since the only `[data-city-card]` elements live there — highlighting a city no longer scrolls the page.
- Added four live per-phase countdown tiles (Antes/Parcial/Totalidad/Después) in the header against the real Aug 12 event clock, and a per-source × per-phase record-count table in the "Antes, durante y después" section as a quick ingestion-health check for the Aug 11 dry run. Added a fixed astronomic vignette (dark elliptical gradient, top and bottom, `pointer-events: none`) pinned through scroll. Source-legend grid switched from viewport-fragile `auto-fit` to explicit column counts so it never strands a lone item next to a wall of empty space, and each source card now carries the provider's real logo (self-hosted in `docs/assets/`: REE, AEMET, DGT, Google Trends) instead of just a color dot. Swapped the DGT Cámaras icon from a camera to a sun, since the metric is sky brightness, not the camera itself.
- `admin.sh preview [port]` — serves `docs/` locally via `python3 -m http.server`, so viewing frontend changes doesn't need a manually-typed command.
- Split the single technical README into a public-facing `README.md` (semi-technical project overview for GitHub visitors) and an internal `AGENTS.md` (full decision log, ops/handoff detail — gitignored, dev-only, not published).

### Known issues
- Closes the "camera cadence during totality" item from 2026-08-09's Known issues — see "Still to decide" removal in README.
- Palma has no DGT camera (only 9 mainland provinces are covered) — by design, not a gap to fix; the dashboard now surfaces this inline instead of looking broken (see "Added" above).
...
- Dashboard globe: static (locked, no auto-rotate) `globe.gl` 3D view framed on the mainland totality path, self-hosted higher-res Earth texture (NASA Blue Marble; the stock 2048px CDN one was blurry at this zoom) and an umbra shadow band along the path. Replaced per-value 3D cylinder markers — hard to compare at this camera angle — with flat color/size-coded dots across **3 switchable views** (Eclipse effect, Weather, Human activity) instead of 5 raw per-source toggles; REE/Trends have no coordinates of their own, so they're plotted as a badge at a fixed mainland reference point instead of being left off the map. Two real bugs found along the way: container measured 0×0 before layout settled, so the globe fell back to full-window size (fixed by pinning size + `ResizeObserver`); disabling OrbitControls in the same tick as `pointOfView()` froze the camera mid-transition at the default wide pose (fixed by deferring the lock via `onGlobeReady` + two `requestAnimationFrame`s).
- Added the official IGN totality map and partial-phase progression image to the dashboard, cropped from the user's local assets (programmatically found the legend-panel boundary rather than eyeballing it) and credited inline to `astronomia.ign.es/eclipses-de-sol-y-luna/eclipse-total-sol-de-12-de-agosto-2026` to avoid copyright issues.
...
- Frontend frozen to a static embedded snapshot (no live polling) for globe/layout dev — **must** switch back to live `fetch()` before the Aug 11 dry run (see README, "Still to decide").
- `file://` blocks WebGL texture loads as cross-origin (confirmed in real Chrome) — local testing needs `python3 -m http.server` from `docs/`; GitHub Pages (https) is unaffected.
